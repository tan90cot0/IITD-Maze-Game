...
bool loadMedia()
{
	...
	if( !gBGTexture87.loadFromFile( "images/yulu.png" ) )
	{
		printf( "Failed to load background texture!\n" );
		success = false;
	}
	if( !gBGTexture88.loadFromFile( "images/yulu.png" ) )
	{
		printf( "Failed to load background texture!\n" );
		success = false;
	}
	...
	else
	{
		...
		{	
			...
			while( !quit )
			{
				...
				bool at_lhc = dot.mPosX>970 && dot.mPosX<1020 && dot.mPosY > 315 && dot.mPosY <370;
				...
				bool at_bharti = dot.mPosX>703 && dot.mPosX<711 && dot.mPosY > 277 && dot.mPosY <298;
				bool at_main = dot.mPosX> 816 && dot.mPosX<825 && dot.mPosY > 161 && dot.mPosY <186;
				...
				timeText1.str( "" );
				int time = (SDL_GetTicks() - startTime)/20;
				...
				if(at_lhc || at_lib)
				{
					if(entered_emo==false)
					{
						startTime_emo = SDL_GetTicks();
						entered_emo = true;
					}
					else
						emo_time = leftover_emo + (SDL_GetTicks() - startTime_emo)/20;
				} 
				else
				{
					if(entered_emo==true)
					{
						leftover_emo = emo_time;
						entered_emo = false;
					}
				}
				timeText2.str( "" );
				timeText2 << "X: " <<dot.mPosX<<"   Y:"<<dot.mPosY;
				...
				if (dot.mPosX <= 180 && dot.mPosX >= 130 && dot.mPosY <=242 && dot.mPosY >= 229 && e.key.keysym.sym == SDLK_SPACE)
					menu = true;
				if (dot.mPosX >= 180 || dot.mPosX <= 130 || dot.mPosY >=242 || dot.mPosY <= 229 || (menu == true && e.key.keysym.sym == SDLK_ESCAPE))
					menu = false;
				...
				dot2.render();
				dot3.render();
				gBGTexture1.render( 565, 270, &camera1 );
				gBGTexture2.render( 1130, 70, &camera2 );		//kailash
				gBGTexture3.render( 1185, 40, &camera3 );		//himadri
				gBGTexture4.render( 770, 260, &camera4 );		//library
				gBGTexture5.render( 970, 300, &camera5 );//remove background properly
				gBGTexture24.render( 430, 220, &camera24 );		//rajdhani
				gBGTexture25.render( 770, 245, &camera25 );		//amul
				gBGTexture26.render( 300, 450, &camera26 );		//sac
				...
				if(at_kailash)
					...
					gPromptTextTexture2.loadFromRenderedText( "You are now in IIT Hospital", textColor );
				else if(at_rajdhani)
					...
					gPromptTextTexture2.loadFromRenderedText( "You are now in Pharmacy", textColor );
				else if(at_delhi16)
				...
				if (menu == false){
				gTimeTextTexture1.render(  1050, 640 );
				gTimeTextTexture2.render(  1050, 670 );
				gTimeTextTexture3.render(  1050, 700 );
				gTimeTextTexture4.render(  1050, 610 );
