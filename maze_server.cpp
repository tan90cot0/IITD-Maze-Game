...
void Dot::handleEvent( SDL_Event& e , Dot dot, bool yulu_mode)
{
	...
	if (((mPosX >=1055 && mPosX <= 1080 && mPosY >=75 && mPosY <= 105) || (mPosX >=408 && mPosX <= 458 && mPosY >=375 && mPosY <= 380) || (mPosX >=941 && mPosX <= 975 && mPosY >=290 && mPosY <= 330) || (mPosX >=180 && mPosX <= 240 && mPosY >=480 && mPosY <= 600)) && yulu == false && yulu_mode == false){       //audio
	...
	if (mPosX >=755 && mPosX <= 800 && mPosY >=360 && mPosY <= 400)          //audio
...
bool loadMedia()
{
	...
	gWaitTexture = loadTexture( "images/waiting.png" );
...
int main( int argc, char* argv[] )
{
	...
	else
	{
		...
		{	
			...
			if (temp == 5){
				SDL_RenderClear( gRenderer );
				SDL_RenderCopy( gRenderer, gWaitTexture, NULL, NULL ); 
				SDL_RenderPresent( gRenderer );
				SDL_DestroyTexture(gWaitTexture);
				gWaitTexture = NULL;
				if (argc < 2) {
			...
			while( !quit )
			{
				...
				if (temp == 5){
					...
					buffer[1] = dot.mPosY;
					buffer[2] = score;
					n = write(newsockfd,buffer,12);
					if (n < 0) 
						error("ERROR writing to socket");
					else
						printf("Write as server: %d %d\n", buffer[0], buffer[1]);
				}
				...
				gTimeTextTexture12.render(  598, 725 );
				gTimeTextTexture13.render(  598, 725 );
	...
	label5: if (temp != 5) close(3,0);
			else if (score2>score) close(2,0);
			else	close(1,0);
